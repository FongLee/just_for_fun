...
queue<int> childfd;
int max_queue = 0;
int max_current_threads = MINTHREADS;
int max_idle_threads = MINTHREADS;
...
int main(int argc, char **argv)
{
	...
	int socket_fd;
	res = srv_socket_init(&socket_fd, BACKLOG, PORT);
	if (res != 0)
	...
	tv.tv_sec = WAITTIME;
	tv.tv_usec = 0;
	res = setsockopt(socket_fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));
	if (res < 0)
		err_sys("setsockopt err");
	res = setsockopt(socket_fd, SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv));
	if (res < 0)
		err_sys("setsockopt err");
	...
	while(1)
	{
		...
		childfd.push(myfd);
		if ((int)childfd.size() > max_queue)
			max_queue = childfd.size();
		...
		{			
				...
				my_pool.curr_threads += k;

				//err_ret("make   pthread");
		}
		
		if(max_current_threads < my_pool.curr_threads)
			max_current_threads = my_pool.curr_threads;
		
		if (max_idle_threads < my_pool.idle_threads)
			max_idle_threads = my_pool.idle_threads;
...
void connection_handler(int newfd)
{
	...
	while(1)
	{
		...
		res = srv_socket_rev(newfd, rec_buf, &buflen, WAITTIME);
		if (res != 0)
		{
			int k = 0;
		...
		else if (res == 0)
		{
			...
			for(int i = 0; i < 10; i++)
			{
				srv_socket_send(newfd, send_buf, sizeof(send_buf), WAITTIME);
				if (res != 0)
				{
					//because time out,continue to receive buffer
					if (res == SOCKET_TIMEOUT_ERR)
...
void my_handler(int num)
{
	...
	if (num == SIGINT)
	{
		...
		fprintf(stdout, "max number of  current thread is %d, max number of idle_threads is %d , max size of queue is %d\n" ,\	
							max_current_threads, \
								max_idle_threads,  max_queue);
