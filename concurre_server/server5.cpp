...
typedef struct user_data
{
	...
	char middle_buffer[BUFFERMAX+1];
} user_data_t;
...
{	
	int status;
	user_data_t clients;
	int (*callback)( struct epoll_event * pev);
	time_t last_alive;
} epoll_event_t;

typedef struct event_pool
{
	epoll_event_t *myepoll;
	int event_len;
	int epoll_fd;
...
typedef struct
{
	int thread_flag; //0: idle; 1: receive ptread; 2: send pthread; 3: work pthread
...
typedef struct thread_pool
{
	...
	pthread_cond_t cond_work; //send to work thread
	queue<int , 
} thread_pool_t;

const int MAXEVENTS = 500
const int PORT = 1234
const int LISTENNUM = 10
...
event_pool_t myevent_pool;
thread_pool_t mythread_pool;
...
int myevent_pool_init();
int mythread_pool_init();
int srv_init();
int search_idle(event_pool_t *pevpool);
int myaccept(epoll_event_t * pev);
int recv_data(epoll_event_t * pev);
int send_data(epoll_event_t *pev);
void thread_make(int i, int flags); // flags:  
